...
namespace CryptoTradingSystem.Scraper
{
    public static class ApiManager
    {
        ...
        public static async Task<List<Quote>> GetBinanceData(Enums.Assets asset, Enums.TimeFrames timeFrame, int limit = 750)
        {
            var quotes = new List<Quote>();
            var assetName = asset.GetStringValue()?.ToUpper();

            var url = $"https://api.binance.com/api/v3/klines?symbol={assetName}&interval={timeFrame.GetStringValue()}&limit={limit}";
