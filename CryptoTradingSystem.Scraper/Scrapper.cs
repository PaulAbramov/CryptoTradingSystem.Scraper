			...
			foreach (var timeFrame in (Enums.TimeFrames[]) Enum.GetValues(typeof(Enums.TimeFrames)))
			{
				...
				for (var iteratingYear = StartYear; iteratingYear <= _currentYear; iteratingYear++)
				{
					...
					for (var iteratingMonth = StartMonth; iteratingMonth <= 12; iteratingMonth++)
					{
						...
						if (data.Length != 0)
						{
							var reader = FileHandler.GetCSVFileStreamReader(data);
							if (reader != null)
							{
								...
								while ((dataToRead = reader.ReadLine()) != null)
								{
									...
									var dateTimeOpen =
										DateTimeOffset.FromUnixTimeMilliseconds(Convert.ToInt64(separatedstrings[0]));
									var dateTimeClose =
										DateTimeOffset.FromUnixTimeMilliseconds(Convert.ToInt64(separatedstrings[6]));
